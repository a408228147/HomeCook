 ...
 var check = false;
	...
	$.each(city,function(i,n) {
		...
		$('#province').change(function() {
			...
			$.each(city[$('#province').get(0).selectedIndex-1].city,function(j,o) {
				$('<option>',{
					text:city[$('#province').get(0).selectedIndex-1].city[j].name,
					val:city[$('#province').get(0).selectedIndex-1].city[j].name,
					style:'padding: 2px; padding-left: 10px'
				}).appendTo('#city');
			})
		});
		$('#city').change(function() {
			...
			$.each(city[$('#province').get(0).selectedIndex-1].city[$('#city').get(0).selectedIndex-1].area,function(k,p) {
				$('<option>',{
					text:city[$('#province').get(0).selectedIndex-1].city[$('#city').get(0).selectedIndex-1].area[k],
					val:city[$('#province').get(0).selectedIndex-1].city[$('#city').get(0).selectedIndex-1].area[k],
					style:'padding: 2px; padding-left: 10px'
				}).appendTo('#area');
			})
		});
	...
	$('#area').change(function() {
		...
		$('#place').keyup(function() {
			$('.in').attr('value',$('#province').val()+$('#city').val()+$('#area').val()+$('#place').val());
		})
	});
		...
		if($('#telephone').val()=="") {
			...
			if(check) {
				$.ajax({
				    ...
				    dataType:'json',
				    // 验证文件
				    url: '../User/CompleteUserInfo.do',
				    // 用户输入的帐号密码
				    data: {'user_name':$('.username').val(), 'user_phone': $("#telephone").val(), 'user_address': $(".in").val(), 'user_introduce': $(".self_introduce").val() },
				    // 异步，不写默认为True
				    async: true,
				    //请求成功后的回调
